    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                console.log("❌ Sesi Logout/Invalid. Hapus folder auth_info_baileys dan mulai ulang.");
                process.exit(1); 
            }
...
function startApp() {
    ...
    server.on('error', (e) => {
        if (e.code === 'EADDRINUSE') {
            console.log(`⚠️ Port ${PORT} sedang digunakan. Server webhook dilewati (Bot WA tetap jalan).`);
        } else {
