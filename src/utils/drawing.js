...
/**
 * 🚀 HYBRID RENDERING ENGINE v4.2 (FINAL STABLE)
 * Stack: Skia (Atmosphere) + Resvg (Vector) + Sharp (Compositor)
 */
...
async function renderBaseLayer(width, height, isReal) {
  ...
  // Lighting
  const primaryColor = isReal ? "#FFD700" : "#00d2ff";
  const secondaryColor = isReal ? "#FF4500" : "#7B68EE";
...
function renderVectorLayer(width, height, user, jid, rankMeta) {
  ...
  const accLabel = isReal ? "PLATINUM / REAL" : "SANDBOX / DEMO";
  const accColor = isReal ? "#F7BF64" : "#A0A0A0";
  ...
  const svg = `
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate(100, 420)">
            ...
            <text x="700" y="30" class="font-val">${user.positions?.length || 0} POSITIONS</text>
        </g>
...
async function createProfileCard(user, jid) {
  ...
  try {
    const baseBuffer = await renderBaseLayer(width, height, isReal);
    const overlayBuffer = renderVectorLayer(width, height, user, jid, rankMeta);
...
/**
 * 2. MARKET CHART REALTIME
 * Membaca data history dari file JSON dan menggambarnya
 */
async function createMarketChart() {
  ...
  // 1. LOAD DATA DARI FILE JSON WEB
  ...
  // Dummy Data
  if (history.length < 2)
    history = [{ p: 1000 }, { p: 1050 }, { p: 1020 }, { p: 1100 }];
  ...
  const isUp = trend.includes("NAIK") || trend.includes("PUMP");
  ...
  fillGrad.addColorStop(
    0,
    isUp ? "rgba(14, 203, 129, 0.5)" : "rgba(246, 70, 93, 0.5)",
  );
