...
async function performMine(ctx) {
    ...
    const reward = Math.floor(Math.random() * (MINING_REWARD_MAX - MINING_REWARD_MIN + 1)) + MINING_REWARD_MIN;
...
async function performDaily(ctx) {
    ...
    if (now - lastDaily < oneDay) {
        return ctx.sock.sendMessage(ctx.from, { text: "❌ Absen harian sudah diambil hari ini. Kembali besok!" });
    }
